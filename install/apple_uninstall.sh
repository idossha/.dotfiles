...
########################################
# dotfiles macOS uninstallation script of Ido Haber
...
OS="$(uname)"
if [ "$OS" != "Darwin" ]; then
  echo "ERROR: This script is for macOS only. For Linux, use linux_uninstall.sh"
  exit 1
fi
...
uninstall_from_manifest() {
  ...
  # Remove snap packages (though unlikely on macOS, keeping for compatibility)
...
main() {
  print_message "Dotfiles macOS Uninstallation Script"
  echo "OS: $OS"
  echo ""
  ...
  echo "This script will uninstall components that were installed by the macOS dotfiles setup."
  ...
  echo "Your macOS system has been restored to its pre-installation state."
  echo "You may need to restart your terminal for all changes to take effect."
  echo ""
  echo "Note: Homebrew and some system packages may still be installed as they might be used by other software."
}

...
#
# The model: content here is canonical and version-controlled; the home directory holds symlinks
# back to it. A destination that already exists as a real file or directory is moved aside with a
# timestamped .backup suffix before the link is made, never deleted. Runtime state (auth, sessions,
# caches, logs) is never touched.
#
...
Runtime state is intentionally left local. --check validates and links nothing.
USAGE
}
...
render_codex_mcp() {
  python3 - "$MCP_SRC" "$CODEX_SRC/config.toml" <<'PY'
import json, sys
mcp_path, toml_path = sys.argv[1], sys.argv[2]
...
if start in text and end in text:
    ...
    new = text.rstrip("\n") + "\n\n" + block + "\n"
if new != text:
    open(toml_path, "w").write(new)
    print("  [ok] Codex MCP block regenerated from mcp-servers.json")
else:
    print("  [ok] Codex MCP block up to date")
PY
}
...
sync_codex() {
  render_codex_mcp
  link_path "Codex config" "$CODEX_SRC/config.toml" "$HOME/.codex/config.toml"
  link_path "Codex rules" "$CODEX_SRC/rules" "$HOME/.codex/rules"
}

...
5. Run a bounded live trial for changed execution behavior. Record its outcome and untested
   capabilities explicitly; a launched process is not a completed one.

MCP network health, account authentication, provider model availability, skill activation quality,
and end-to-end no-mistakes delivery are separate from offline configuration checks.
...
| `quota-axi` | Local quota/usage visibility before routing work to an expensive model. | Data-only; it does not choose the model by itself. |
...
- `agentctl ship` refuses a project: the repository has no reviewed `.no-mistakes.yaml`, or the branch is
  not clean. Both are deliberate; do not bypass them by pushing directly.
- Pi print mode hangs: `pi -p` reads stdin until EOF, so redirect scripted runs from `/dev/null`.

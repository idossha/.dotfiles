...
- High-pass before ICA, often around 1 Hz, for decomposition quality.
...
- After manual `EEG` struct edits, run `eeg_checkset(EEG)`.

## When Reporting

Be concrete about the dataset stage, reference scheme, filtering choices, ICA assumptions, and which `pop_*` functions should be used.

## Menu-to-Function Reference

See the full mapping in [menu-reference.md](menu-reference.md).

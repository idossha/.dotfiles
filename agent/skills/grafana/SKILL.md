...
1. Identify the audience: operator, developer, on-call, product, or executive.
2. Identify the signal type: metrics, logs, traces, uptime, or business KPIs.
3. Design top-down: status and burn first, core time series next, then breakdowns and drilldowns.
...
- Use meaningful panel titles: `API error rate`, not `Panel 12`.
- Show units everywhere.
- Use templating variables sparingly.
- Put thresholds on stat panels only when thresholds are operationally meaningful.
...
- Edit dashboard JSON or provisioning files directly when the user provides them.
